	...
	worker := func() {
		...
		for j := range paths {
			...
			if *skipUnchanged && unchanged && !lastScanOld {
				...
				outF.Write([]byte("\n"))
				if csvW != nil {
					_ = csvW.Write([]string{res.Path, fmt.Sprintf("%t", res.NSFW), fmt.Sprintf("%.6f", res.NSFWScore), fmt.Sprintf("%.6f", res.Threshold), "0"})
				}
				mu.Unlock()
				continue
			}
			...
			if needHash || lastScanOld {
				if s, e := sha256OfFile(p); e == nil {
					sumHex = s
				} else {
					log.Printf("hash fail %s: %v", p, e)
				}
			}
			if sumHex != "" && rec == nil {
